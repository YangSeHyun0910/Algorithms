...
public class Stage2_5 {
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        //입력받은 시(H), 분(M)이 if()문 조건에 맞아야한다.
        if (H >= 0 && H <= 23 && M >= 0 && M <= 59) {

            //입력받은 분(M)이 45보다 작다면, 시(H)에서 -1을 하면서, 분(M)에 60이 더해져야한다
            if (M - 45 < 0) {
                H -= 1;
                M += 60 - 45;

                //시(H)에서 1이 빼지면서 시(H)값이 0보다 작아지면, 23시가 되기 때문에 23을 할당.
                if (H < 0) {
                    H = 23;
                }
                System.out.println(H + " " + M);
            } else {
                //입력받은 분(M) - 45가 0보다 크다면 시(H)값을 -1 할 필요없다.
                M -= 45;
                System.out.println(H + " " + M);
            }
        }
